...
locals {
  ...
  etcd_private_ips = "${slice(var.private_network_ips, 0, var.node_count)}"
}
...
data "template_file" "etcd-service" {
  ...
  vars = {
    ...
    intial_cluster        = "${join(",", formatlist("%s=http://%s:2380", local.etcd_hostnames, local.etcd_private_ips))}"
    listen_client_urls    = "http://${element(local.etcd_private_ips, count.index)}:2379"
    advertise_client_urls = "http://${element(local.etcd_private_ips, count.index)}:2379"
    listen_peer_urls      = "http://${element(local.etcd_private_ips, count.index)}:2380"
  }
}
